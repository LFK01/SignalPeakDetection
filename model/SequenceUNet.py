from keras.layers import Conv1D, BatchNormalization, Activation, Input, Add, UpSampling1D, GlobalAveragePooling1D, \
    Dense, Multiply, Concatenate, AveragePooling1D, TimeDistributed, GRU
...
def SeqUNet(input_shape):
    ...
    def se_block(se_block_layer_input, se_block_layer_n):
        ...
        x_out = TimeDistributed(Multiply()([se_block_layer_input, se_block_layer]))
        return x_out
    ...
    def resblock(resblock_layer_input, resblock_layer_n, kernel, dilation, use_se=True):
        ...
        resblock_layer = TimeDistributed(Add()([resblock_layer_input, resblock_layer]))
    ...
    x = TimeDistributed(Concatenate()([x, input_layer_1]))
    ...
    x = TimeDistributed(Concatenate()([x, out_1]))
    x = cbr(x, layer_n * 3, small_kernel_size, 1, 1)

    x = TimeDistributed(UpSampling1D(4))(x)
    x = TimeDistributed(Concatenate()([x, out_0]))
    x = cbr(x, layer_n * 2, small_kernel_size, 1, 1)

    # binary classifier
    x = TimeDistributed(Conv1D(1, kernel_size=small_kernel_size, strides=1, padding="same"))(x)
    out = TimeDistributed(Activation('sigmoid'))(x)
